...
namespace LifeCrm.Web.Services
{
    public class ApiClient
    {
        ...
        private async Task<ApiResponse<T>> GetAsync<T>(string url)
        {
            ...
            try { return await _http.GetFromJsonAsync<ApiResponse<T>>(url, _jsonOpts) ?? ApiResponse<T>.Fail("Empty response."); }
        ...
        private async Task<ApiResponse<T>> PostAsync<T>(string url, object? body = null)
        {
            ...
            try
            {
                var resp = body is null ? await _http.PostAsync(url, null) : await _http.PostAsJsonAsync(url, body, _jsonOpts);
                return await resp.Content.ReadFromJsonAsync<ApiResponse<T>>(_jsonOpts) ?? ApiResponse<T>.Fail("Empty response.");
            }
            catch (Exception ex) { return ApiResponse<T>.Fail(ex.Message); }
        }
        ...
        private async Task<ApiResponse> PostVoidAsync(string url, object? body = null)
        {
            ...
            try
            {
                ...
                return await resp.Content.ReadFromJsonAsync<ApiResponse>(_jsonOpts) ?? ApiResponse.Fail("Unknown error.");
            }
        ...
        private async Task<ApiResponse> DownloadFileAsync(string url, string filename)
        {
            await AttachTokenAsync();
            try
            {
                var bytes = await _http.GetByteArrayAsync(url);
                await _js.InvokeVoidAsync("lifecrm.downloadFile", filename, Convert.ToBase64String(bytes));
        ...
        public async Task<ApiResponse<LoginResponse>> LoginAsync(LoginRequest request)
        {
            ...
            return await resp.Content.ReadFromJsonAsync<ApiResponse<LoginResponse>>(_jsonOpts) ?? ApiResponse<LoginResponse>.Fail("Connection error.");
        }
        public async Task<ApiResponse> ChangePasswordAsync(ChangePasswordRequest request) => await PostVoidAsync("api/v1/auth/change-password", request);
        ...
        public async Task<ApiResponse<ImportContactsResult>> ImportContactsCsvAsync(Stream fileStream, string fileName)
        {
            ...
            try
            {
                ...
                return await resp.Content.ReadFromJsonAsync<ApiResponse<ImportContactsResult>>(_jsonOpts) ?? ApiResponse<ImportContactsResult>.Fail("Import failed.");
            }
            catch (Exception ex) { return ApiResponse<ImportContactsResult>.Fail(ex.Message); }
        }

        // DONATIONS
        public async Task<ApiResponse<PagedResult<DonationListDto>>> GetDonationsAsync(PaginationParams p, Guid? contactId = null, DateOnly? fromDate = null, DateOnly? toDate = null)
        {
        ...
        public async Task<ApiResponse<DocumentDto>>  GenerateReceiptAsync(Guid id, bool sendByEmail)         => await PostAsync<DocumentDto>($"api/v1/donations/{id}/receipt?sendByEmail={sendByEmail}");
        public async Task<ApiResponse>               DownloadReceiptAsync(Guid donationId, Guid documentId)  => await DownloadFileAsync($"api/v1/donations/{donationId}/receipt/{documentId}/download", $"receipt-{donationId:N}.pdf");
        public async Task<ApiResponse>               DownloadLatestReceiptAsync(Guid donationId)             => await DownloadFileAsync($"api/v1/documents/donation/{donationId}/receipt/latest", $"receipt-{donationId:N}.pdf");
        ...
        public async Task<ApiResponse<CampaignDto>> GetCampaignAsync(Guid id)                              => await GetAsync<CampaignDto>($"api/v1/campaigns/{id}");
        public async Task<ApiResponse<Guid>>        CreateCampaignAsync(CreateCampaignRequest req)          => await PostAsync<Guid>("api/v1/campaigns", req);
        public async Task<ApiResponse>              UpdateCampaignAsync(Guid id, UpdateCampaignRequest req) => await PutAsync($"api/v1/campaigns/{id}", req);
        public async Task<ApiResponse>              DeleteCampaignAsync(Guid id)                            => await DeleteAsync($"api/v1/campaigns/{id}");
        ...
        // INTERACTIONS
        public async Task<ApiResponse<IReadOnlyList<InteractionDto>>> GetInteractionsAsync(Guid contactId) => await GetAsync<IReadOnlyList<InteractionDto>>($"api/v1/contacts/{contactId}/interactions");
        public async Task<ApiResponse<Guid>> CreateInteractionAsync(CreateInteractionRequest req)           => await PostAsync<Guid>("api/v1/interactions", req);
        public async Task<ApiResponse>       UpdateInteractionAsync(Guid id, UpdateInteractionRequest req)  => await PutAsync($"api/v1/interactions/{id}", req);
        public async Task<ApiResponse>       DeleteInteractionAsync(Guid id)                                => await DeleteAsync($"api/v1/interactions/{id}");

        // USERS
        public async Task<ApiResponse<IReadOnlyList<UserSummaryDto>>> GetUsersAsync()                    => await GetAsync<IReadOnlyList<UserSummaryDto>>("api/v1/users");
        public async Task<ApiResponse> ChangeUserRoleAsync(Guid id, UserRole newRole)                    => await PatchAsync($"api/v1/users/{id}/role", newRole);
        public async Task<ApiResponse> DeactivateUserAsync(Guid id)                                      => await PatchAsync($"api/v1/users/{id}/deactivate", new { });
        public async Task<ApiResponse> ActivateUserAsync(Guid id)                                        => await PatchAsync($"api/v1/users/{id}/activate", new { });

        // DASHBOARD
        public async Task<ApiResponse<DashboardDto>> GetDashboardAsync() => await GetAsync<DashboardDto>("api/v1/dashboard");

        // DOCUMENTS
        public async Task<ApiResponse<DocumentDto>> GenerateDonationSummaryAsync(GenerateDonationSummaryRequest req) => await PostAsync<DocumentDto>("api/v1/documents/summary", req);
        public async Task<ApiResponse>              DownloadDocumentAsync(Guid id) => await DownloadFileAsync($"api/v1/documents/{id}/download", $"document-{id:N}.pdf");
    }
}
