...
builder.Services
    ...
    .AddJwtBearer(opts =>
    {
        ...
        {
            ValidateIssuer = true,
            ValidIssuer = jwtSettings["Issuer"] ?? "LifeCrm",
            ValidateAudience = true,
            ValidAudience = jwtSettings["Audience"] ?? "LifeCrmWeb",
...
builder.Services.AddRateLimiter(opts =>
{
    ...
    {
        o.Window = TimeSpan.FromMinutes(5); o.PermitLimit = 10;
        o.QueueLimit = 0; o.QueueProcessingOrder = QueueProcessingOrder.OldestFirst;
    });
    opts.AddFixedWindowLimiter("api", o =>
    {
        o.Window = TimeSpan.FromMinutes(1); o.PermitLimit = 300;
        o.QueueLimit = 0; o.QueueProcessingOrder = QueueProcessingOrder.OldestFirst;
    });
...
// ── Middleware pipeline ───────────────────────────────────────
// Order matters: ExceptionMiddleware → Security → HTTPS → Static →
//               Routing → CORS → RateLimit → Auth → Tenant → AuthZ → Endpoints

app.UseMiddleware<ExceptionMiddleware>();
app.UseMiddleware<SecurityHeadersMiddleware>();  // FIXED: was commented out
...
if (app.Environment.IsDevelopment())
{
    ...
    app.UseSwaggerUI(c =>
    {
        c.SwaggerEndpoint("/swagger/v1/swagger.json", "LifeCrm API v1");
