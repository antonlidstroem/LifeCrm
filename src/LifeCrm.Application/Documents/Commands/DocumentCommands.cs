...
namespace LifeCrm.Application.Documents.Commands
{
    ...
    {
        private readonly IUnitOfWork _uow; private readonly IPdfService _pdf;
        private readonly IEmailService _email; private readonly ICurrentUserService _cu;
        private readonly IOrganizationReader _orgReader;

        public GenerateDonationReceiptHandler(IUnitOfWork uow, IPdfService pdf, IEmailService email, ICurrentUserService cu, IOrganizationReader or)
        { _uow = uow; _pdf = pdf; _email = email; _cu = cu; _orgReader = or; }
        ...
        public async Task<DocumentDto> Handle(GenerateDonationReceiptCommand cmd, CancellationToken ct)
        {
            ...
            _uow.Donations.Update(donation); await _uow.SaveChangesAsync(ct);

            if (req.SendByEmail && !string.IsNullOrWhiteSpace(contact.Email) && !contact.EmailOptOut)
            {
                try { await _email.SendAsync(contact.Email, contact.Name, $"Donation Receipt from {org.Name}", $"<p>Dear {contact.Name},</p><p>Thank you for your donation of ${donation.Amount:N2}.</p>", pdfBytes, fileName, ct); }
                catch { }
            }
    ...
    public sealed class GenerateDonationSummaryHandler : IRequestHandler<GenerateDonationSummaryCommand, DocumentDto>
    {
        ...
        public GenerateDonationSummaryHandler(IUnitOfWork uow, IPdfService pdf, IEmailService email, ICurrentUserService cu, IOrganizationReader or)
        { _uow = uow; _pdf = pdf; _email = email; _cu = cu; _orgReader = or; }
        ...
        public async Task<DocumentDto> Handle(GenerateDonationSummaryCommand cmd, CancellationToken ct)
        {
            ...
            await _orgReader.SaveDocumentAsync(doc, ct);
            if (req.SendByEmail && !string.IsNullOrWhiteSpace(contact.Email) && !contact.EmailOptOut)
            { try { await _email.SendAsync(contact.Email, contact.Name, $"Donation Summary from {org.Name}", $"<p>Please find your donation summary attached.</p>", bytes, fn, ct); } catch { } }
