    ...
    public sealed class ImportContactsHandler : IRequestHandler<ImportContactsCommand, ImportContactsResult>
    {
        ...
        public async Task<ImportContactsResult> Handle(ImportContactsCommand command, CancellationToken cancellationToken)
        {
            ...
            foreach (var row in rows)
            {
                ...
                var contact = new Contact
                {
                    Id = Guid.NewGuid(), OrganizationId = orgId, Name = row.Name.Trim(),
                    Email = row.Email?.Trim().ToLowerInvariant(), Phone = row.Phone?.Trim(),
                    Tags = row.Tags?.Trim(), AddressLine1 = row.AddressLine1?.Trim(),
                    City = row.City?.Trim(), StateProvince = row.StateProvince?.Trim(),
                    PostalCode = row.PostalCode?.Trim(), Country = row.Country?.Trim(), Notes = row.Notes?.Trim()
                };
                await _uow.Contacts.AddAsync(contact, cancellationToken);
                successCount++;
            }
