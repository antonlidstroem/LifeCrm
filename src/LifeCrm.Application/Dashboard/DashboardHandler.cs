...
namespace LifeCrm.Application.Dashboard
{
    ...
    public sealed class GetDashboardHandler : IRequestHandler<GetDashboardQuery, DashboardDto>
    {
        ...
        public async Task<DashboardDto> Handle(GetDashboardQuery q, CancellationToken ct)
        {
            ...
            var newCon    = await _uow.Contacts.CountAsync(c => c.CreatedAt >= thisMonthStart.ToDateTime(TimeOnly.MinValue), ct);
            var active    = await _uow.Campaigns.GetActiveAsync(ct);
            var ids       = active.Select(c => c.Id).ToList();
            var totals    = await _uow.Donations.Query().Where(d => d.CampaignId.HasValue && ids.Contains(d.CampaignId.Value))
                .GroupBy(d => d.CampaignId!.Value).Select(g => new { g.Key, Total = g.Sum(d => (decimal?)d.Amount) ?? 0 }).ToListAsync(ct);
            var totDict   = totals.ToDictionary(x => x.Key, x => x.Total);

            var top = active.Take(5).Select(c =>
            {
                var r = totDict.GetValueOrDefault(c.Id, 0m);
                return new CampaignSummaryDto { Id = c.Id, Name = c.Name, BudgetGoal = c.BudgetGoal, TotalRaised = r,
                    ProgressPercent = c.BudgetGoal.HasValue && c.BudgetGoal > 0 ? Math.Round(r / c.BudgetGoal.Value * 100, 1) : null };
            }).ToList().AsReadOnly();

            var recentDon = await _uow.Donations.Query().OrderByDescending(d => d.CreatedAt).Take(10)
                .Select(d => new ActivityFeedItemDto { ActivityType = "Donation", EntityId = d.Id, ContactId = d.ContactId,
                    ContactName = d.Contact != null ? d.Contact.Name : string.Empty,
                    Summary = "$" + d.Amount.ToString("N2") + " donation", OccurredAt = d.CreatedAt }).ToListAsync(ct);

            var recentInt = await _uow.Interactions.Query().Where(i => i.ContactId.HasValue).OrderByDescending(i => i.OccurredAt).Take(10)
                .Select(i => new ActivityFeedItemDto { ActivityType = "Interaction", EntityId = i.Id,
                    ContactId = i.ContactId!.Value, ContactName = i.Contact != null ? i.Contact.Name : string.Empty,
                    Summary = i.Type.ToString() + " logged", OccurredAt = i.OccurredAt }).ToListAsync(ct);

            var feed = recentDon.Concat(recentInt).OrderByDescending(a => a.OccurredAt).Take(10).ToList().AsReadOnly();

            return new DashboardDto
            {
                DonationsThisMonth = thisMonth, DonationsLastMonth = lastMonth, DonationsMoMChangePercent = moM,
                TotalContacts = contacts, NewContactsThisMonth = newCon, ActiveCampaigns = active.Count,
                TopCampaigns = top, RecentActivity = feed
            };
        }
    }
}
